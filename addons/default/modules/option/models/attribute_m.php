...
include "base_m.php";
class Attribute_m extends Base_m
{
	protected $_table = 'tshirt_option_attribute';	
	protected $_descriptions="tshirt_options_attribute_lang";
	protected $_primary="id_option";
	...
	public function auto_delete($id){
		...
		if($this->db->delete($this->_table)){
			return	$this->db->where_in($this->_primary,($id))->delete($this->_descriptions);
	...
	public function get_options($params,$offset=0,$limit=6){
	

		$this->db->select("SQL_CALC_FOUND_ROWS *", FALSE);
		if(isset($params['id_option']) && intval($params['id_option']) ){
			$this->db->where("id_option",intval($params['id_option']));
		}
		
		$this->db->join($this->_descriptions,$this->_descriptions.'.'.$this->_primary.'='.$this->_table.'.'.$this->_primary);
	...
	public function get($id){
		...
		$this->db->select("*");
		$this->db->join($this->_descriptions,$this->_descriptions.'.'.$this->_primary.'='.$this->_table.'.'.$this->_primary,"LEFT");
		$this->db->where('lang_code',CURRENT_LANGUAGE);
		$this->db->where('deleted',0);
