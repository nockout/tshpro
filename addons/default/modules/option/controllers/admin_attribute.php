 ...
 */
class Admin_Attribute extends Admin_Controller
{
	...
	public function index($id_option=0,$page=0,$limit=6)
	{
	
		$this->load->model("attribute_m");
		$cond=array();
		if($option=intval($id_option)){
			$cond['id_option']=$option;
		}
		$options=$this->option_m->get_options($cond,$page,$limit);
	    $pagination = create_pagination('admin/option/index', $options['total'],5);

		$this->template->set('options',$options['objects'])
