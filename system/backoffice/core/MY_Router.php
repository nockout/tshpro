...
class My_Router extends MX_Router {
    ...
    function _parse_routes() {
        ...
        // now try the site specific routing
        ...
        $str = mb_convert_encoding($segments[0], "SJIS", "auto");
        $segments[0] = urldecode($str);
        
        if (count($segments) == 1) {
            $row = $this->_get_db_route($segments[0]);
			
            if (!empty($row)) {
                return $this->_set_request(explode('/', $row['query']));
            }
        } else {
            ...
            if ($row) {
                if (!$page_flag) {
                    ...
                    if (preg_match('#^' . $key . '$#', $uri)) {
                        $row['route'] = preg_replace('#^' . $key . '$#', $row['query'], $uri);
                        return $this->_set_request(explode('/', $row['query']));
                    }
                }
            }
    ...
    function _get_db_route($slug) {
        ...
        return DB()->where('keyword', $slug)->get('default_url_alias')->row_array();
    }
