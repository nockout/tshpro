<?php defined('BASEPATH') or exit('No direct script access allowed');
/**
 * Chinese Simpplified translation.
 *
 * @author		Kefeng DENG
 * @package		PyroCMS
 * @subpackage 	Template Module
 * @category	Modules
 * @link		http://pyrocms.com
 * @date		2012-06-27
 * @version		1.1
 */

// Labels
$lang['templates:language_label']			= '語言';
$lang['templates:choose_lang_label']		= '選擇語言';
$lang['templates:subject_label']			= '主旨';
$lang['templates:body_label']				= '內容';
$lang['templates:slug_label']				= '縮略名(slug)';

// Titles
$lang['templates:create_title']				= '建立範本';
$lang['templates:edit_title']				= '編輯範本 "%s"';
$lang['templates:clone_title']				= '複製範本 "%s"';
$lang['templates:list_title']				= '範本列表';
$lang['templates:default_title']			= '預設範本';
$lang['templates:user_defined_title']		= '自定範本';

// Messages
$lang['templates:tmpl_create_success']		= '邮件模板 "%s" 已經儲存';
$lang['templates:tmpl_create_error']		= '邮件模板 "%s" 沒有儲存';
$lang['templates:tmpl_edit_success']		= '邮件模板 "%s" 的變更已經儲存。';
$lang['templates:tmpl_edit_error']			= '邮件模板 "%s" 的變更沒有儲存。';
$lang['templates:tmpl_clone_success']		= '"%s" 已經複製。您現在可以開始編輯。';
$lang['templates:tmpl_clone_error']			= '"%s" 無法複製。請再試一次。';
$lang['templates:single_delete_success']	= '這個邮件模板已經刪除。';
$lang['templates:mass_delete_success']		= '%s 個邮件模板已經刪除(共選擇 %s 個範本)。';
$lang['templates:mass_delete_error'] 		= '正當嘗試刪除邮件模板 "%s" 時，有錯誤發生。';
$lang['templates:default_delete_error'] 	= '有錯誤，預設邮件模板不能移除。';
$lang['templates:no_select_error'] 			= '您需要先選取邮件模板。';
$lang['templates:already_exist_error']		= '一個和 "%s" 相同名稱的邮件模板已經存在。';

/* End of file templates_lang.php */